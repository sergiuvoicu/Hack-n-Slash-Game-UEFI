...
// return 1 if ms seconds have elapsed since the last call
		...
		if(enArr[i] != -100 && enArr[i+1] != -100){
			...
			if(enArr[i] < x){
				bullets[i] = enArr[i]+w2;
                	        if(enArr[i+1] == y)
					bullets[i+1] = enArr[i+1];
                    		if(enArr[i+1] < y)
					bullets[i+1] = enArr[i+1]+h2;
                	        if(enArr[i+1] > y)
					bullets[i+1] = enArr[i+1];
               		}
               		else if(enArr[i] == x){
...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE img, EFI_SYSTEM_TABLE *systab) {
  ...
  while( start == 1){
  		...
  		if(key1.UnicodeChar == 'Y'){
			int enemies[nEn];
  			...
  			while (num!=0 && lifes > 0) {
    				tps();
   				if(interval(600) == 1)
					moveEnemies(enemies,blocks,nEn,nBl,10,x,y,w3,h3,w2,h2,width,height,0,&nEn);
    				moveEnemies(bullets,blocks,nEn,nBl,deltaB, xinit,yinit,w3,h3,w4,h4,width, height, 1, &numB);
				// background
				...
				if (key1.UnicodeChar == 'w'){
					y -= delta;
					flag = 1;
				}
    				if (key1.UnicodeChar == 'a'){
					x -= delta;
					flag = 0;
				}
    				if (key1.UnicodeChar == 's'){
					y += delta;
					flag = 1;
				}
    				if (key1.UnicodeChar == 'd'){
					x += delta;
					flag = 0;
				}
    				if (key1.UnicodeChar == 'm')
					deleteEnemy(enemies,nEn,&num,x,y,w2,h2);
   ...
   uefi_call_wrapper(gop->Blt, 10, gop, buffer, EfiBltBufferToVideo,0,0,0,0,width,height,0);
   if(level == 4)
	Print(L"GAME OVER \t YOU WIN \t START OVER? Y/N \n");
   else if ( lifes == 0)
	Print(L"GAME OVER \t YOU LOSE \t TRY AGAIN? Y/N \n");
   WaitForSingleEvent(ST->ConIn->WaitForKey,0);
   uefi_call_wrapper(ST->ConIn->ReadKeyStroke, 2, ST->ConIn, &key1);
